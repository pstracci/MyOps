...
try:
    ...
    if client_path and os.path.isdir(client_path):
        ...
        logging.info(f"Oracle Client inicializado com sucesso. Versao: {oracledb.clientversion()}")
    else:
        logging.warning(f"Caminho para o Oracle Instant Client nao encontrado. Verifique a chave 'instant_client_path'.")
except Exception as e:
    logging.error(f"AVISO: Nao foi possivel inicializar o Oracle Client. Erro: {e}", exc_info=True)


from PyQt6.QtWidgets import QApplication, QMainWindow, QMessageBox, QMdiArea
from PyQt6.QtGui import QFont, QAction, QPixmap, QPainter, QActionGroup, QIcon
...
from modules.siebel_bscs.siebel_bscs_ui import SiebelBscsWidget
from modules.common import themes
from modules.common import license_validator

# --- Código de Verificação de Update ---
import requests
import webbrowser
from packaging import version
from __version__ import __version__

GITHUB_REPO = "pstracci/MyOps"
...
def check_for_updates():
    try:
        ...
        logging.info(f"Versão local: {__version__}, Última versão no GitHub: {latest_version}")
        if version.parse(latest_version) > version.parse(__version__):
            msg_box = QMessageBox(); msg_box.setIcon(QMessageBox.Icon.Information)
            msg_box.setWindowTitle("Nova Versão Disponível!")
            msg_box.setTextFormat(Qt.TextFormat.MarkdownText)
            msg_box.setText(f"### Nova versão ({latest_version}) disponível!\n\nSua versão: **{__version__}**.\n\nDeseja ir para a página de download?")
            msg_box.setStandardButtons(QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No)
            if msg_box.exec() == QMessageBox.StandardButton.Yes: webbrowser.open(release_page_url)
...
class BackgroundMdiArea(QMdiArea):
    def __init__(self, image_path, parent=None):
        ...
        if self.background_pixmap.isNull(): logging.warning(f"Imagem de fundo '{image_path}' não encontrada.")
    def paintEvent(self, event):
        super().paintEvent(event)
        if self.background_pixmap.isNull(): return
        painter = QPainter(self.viewport()); painter.setOpacity(0.3)
...
class MainApplicationWindow(QMainWindow):
    def __init__(self):
        ...
        else: logging.warning(f"Ícone da aplicação não encontrado em: {icon_path}")
    ...
    def _create_menu_bar(self):
        ...
        self.module_map = {
            ...
            "Comparativo Siebel x BSCSIX": (SiebelBscsWidget, "Comparativo Siebel x BSCSIX"),
        ...
        menu_structure = {
            ...
            "Sistemas Legados": ["Comparativo Siebel x BSCSIX", "Consultar Relacionamentos Siebel", "Extrator de Faturas DMS", "Ferramentas PGU", "Monitor Health GFA", "Visualizador de Contestações", "Ferramentas Base Espelho"]
        }
        ...
        self.light_mode_action.triggered.connect(lambda: self.apply_theme('light')); preferences_menu.addAction(self.light_mode_action); theme_group.addAction(self.light_mode_action)
        self.dark_mode_action = QAction("Modo Escuro (Dark)", self, checkable=True)
        self.dark_mode_action.triggered.connect(lambda: self.apply_theme('dark')); preferences_menu.addAction(self.dark_mode_action); theme_group.addAction(self.dark_mode_action)
        help_menu = menu_bar.addMenu("Ajuda")
        about_action = QAction("Sobre", self); about_action.triggered.connect(self.show_about_dialog); help_menu.addAction(about_action)

    def apply_theme(self, theme_name, from_load=False):
        app = QApplication.instance()
        if theme_name == 'dark': app.setStyleSheet(themes.get_dark_theme_qss())
        else: app.setStyleSheet(themes.get_light_theme_qss())
    ...
    def open_connection_manager(self):
        dialog = ConnectionManagerDialog(self); dialog.exec()
        
    def open_settings_dialog(self):
        dialog = SettingsDialog(self); dialog.exec()
        
    def show_about_dialog(self):
        about_text = f"""<b>MyOps - Ferramentas de Automação e Suporte</b>
<p>Versão {__version__}</p><p>© 2025. Todos os direitos reservados.</p>"""
...
if __name__ == '__main__':
    try:
        ...
        QMessageBox.critical(None, "Erro Crítico", f"A aplicação encontrou um erro fatal.\n\nDetalhes: {e}")
        sys.exit(1)